	...
	providers: [
		...
		Credentials({
			...
			async authorize(credentials) {
				...
				try {
					const response = await fetch(
						`${process.env.EXTERNAL_API_BASE_URL}/verify-otp`,
						{
							method: "POST",
							headers: {
								"Content-Type": "application/json",
	...
	callbacks: {
		async signIn({ user, account, profile }) {
			if (account?.provider === "google") {
				try {
					console.log("Google OAuth signIn callback:", {
						user,
						account,
						profile,
					});
					...
					console.log("Sending to IdP server:", {
						...
						access_token: requestBody.access_token
							? `${requestBody.access_token.substring(0, 20)}...`
							: undefined,
					...
					const response = await fetch(
						`${process.env.EXTERNAL_API_BASE_URL}/oauth-login`,
						{
							method: "POST",
							headers: {
								"Content-Type": "application/json",
					...
					if (response.ok) {
						...
						console.error(
							"Failed to authenticate with IdP server:",
							response.status,
						);
						return false;
					}
